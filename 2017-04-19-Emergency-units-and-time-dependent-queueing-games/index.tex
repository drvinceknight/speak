...
\centering
\Huge{Emergency units and time dependent queueing games}
\large{\href{https://twitter.com/drvinceknight}{@drvinceknight}}
...
\frame{
    ...
    \end{center}
    \pause
    \textbf{Mathematical modelling of patient flows to predict critical care capacity required following the merger of two District General Hospitals into one.}, \textit{Anaesthesia}
}
...
\frame{ \textbf{Measuring the Price of Anarchy in Critical Care Unit
    Interactions}, \textit{Journal of Operational Research}
...
\frame{
    ...
    \begin{tikzpicture}
        % A&E
        \draw [fill=red!20] (4,.5) rectangle (6,1.5);
        \node at (5,2) {\color{blue}{$c$}};
        \node at (5,1) {$\mu$};

        % Patients
        \node at (5,-1.75) {$\Lambda$};
        \draw [->] (5,-1.5) -- (5, -.5);
    \end{tikzpicture}
}
...
\frame{
    \begin{center}
        \begin{tikzpicture}[scale=0.8, every node/.style={scale=0.8}]
            ...
            \draw [very thick] (6,-.25) -- (6,.25);
            % Mid ticks
            \draw [thick] (1,-.125) -- (1,.125);
            \draw [thick] (2,-.125) -- (2,.125);
            \draw [thick] (3,-.125) -- (3,.125);
            \draw [thick] (5,-.125) -- (5,.125);
            % Parameters: j
            \node at (.5,.25)  {$j=1$};
            \node at (1.5,.25) {$j=2$};
            \node at (2.5,.25) {$j=3$};
            \node at (5.5,.25) {$j=n$};
            % Parameters: mu, c
            \node at (.5,-.25) [draw, scale=.7] {$ c_1,\mu_1$};
            \node at (1.5,-.25) [draw, scale=.7] {$c_2,\mu_2$};
            \node at (2.5,-.25) [draw, scale=.7] {$c_3,\mu_3$};
            \node at (5.5,-.25) [draw, scale=.7] {$c_n,\mu_n$};
            % Parameters: lambda
            \node at (.5, .75) [draw, scale=.7, ellipse] {$ \lambda_1$};
            \node at (1.5,.75) [draw, scale=.7, ellipse] {$\lambda_2$};
            \node at (2.5,.75) [draw, scale=.7, ellipse] {$\lambda_3$};
            \node at (5.5,.75) [draw, scale=.7, ellipse] {$\lambda_n$};
            % Ellipsis
            \node at (4,0) {$\dots$};
            % Underbrace
            \draw [ thick, decoration={ brace, mirror, raise=0.5cm }, decorate ] (0,0) -- (6,0) node [pos=.5, anchor=north, yshift=-.75cm] {$T$};
        \end{tikzpicture}
    \end{center}
}
...
\frame{
	\centering{
			\begin{tikzpicture}
				...
				\draw [thick] (5,-.125) -- (5,.125);
				% Arrival
				\node (arrival) at (.5,0) [draw, circle, fill=blue] {} node [below=.3cm, right] {\tiny arrival};
				% Decisions
				\draw (arrival) edge[out=40,in=135,->] node [above] {?} (1.75,0);
				\draw (arrival) edge[out=65,in=125,->] node [above] {?} (3.25,0);
				\draw (arrival) edge[out=85,in=115,->] node [above] {?} (5.5,0);
				% Ellipsis
				\node at (4,0) {$\dots$};
			\end{tikzpicture}
			}
...
\frame{
	\centering{
			\begin{tikzpicture}
				\node (arrival) at (0,0) [draw, circle, fill=blue] {} node [below=.15cm] {\tiny{arrival}};
				% Dummy states
				\node (d1) at ($(arrival) + (2,2)$) [draw, text width = .5cm, align=center] {?};
				\node (d2) at ($(arrival) + (2,1.2)$) [draw, text width = .25cm, align=center] {?};
				\node (d3) at ($(arrival) + (2,.4)$) [draw] {?};
				% Service station
				\node (service) at ($(arrival) + (5,0)$) [draw] {service};
				% Decisions
				...
				\draw [->] (arrival) -- (d3);
				% Delays
				\draw (d1) edge[out=0,in=180,->,dashed] (service);
				\draw (d2) edge[out=0,in=180,->,dashed] (service);
				\draw (d3) edge[out=0,in=180,->,dashed] (service);
			\end{tikzpicture}
			}
}
...
\frame{
    ...
    \begin{tikzpicture}
        ...
        % Patients
        \node at (3,-1.75) {$\Lambda_1$};
        \draw [->] (3,-1.5) -- (4.5, -.5);
        \node at (5,-1.75) {$\Lambda_2$};
        \draw [->] (5,-1.5) -- (5, -.5);
        \node at (7,-1.75) {$\Lambda_3$};
        \draw [->] (7,-1.5) -- (5.5, -.5);
    \end{tikzpicture}
}
...
\frame{
	\centering
        ...
        % First generation
        \draw (0,-.5) rectangle (3,1.5);
            % Line
            \draw (.25,0) -- (2.75,0);
            % End ticks
            \draw [very thick] (.25,-.25) -- (0.25,.25);
            \draw [very thick] (2.75,-.25) -- (2.75,.25);
            % Mid ticks
            \draw [thick] (1,-.125) -- (1,.125);
            \draw [thick] (2,-.125) -- (2,.125);
            % Dots
                % First node
            \node at (.625,.25) [draw, circle, fill=blue] {};
            \node at (.625,.5) [draw, circle, fill=blue] {};
            \node at (.625,.75) [draw, circle, fill=red] {};
                % Second node
            \node at (1.5,.25) [draw, circle, fill=red] {};
            \node at (1.5,.5) [draw, circle, fill=blue] {};
            \node at (1.5,.75) [draw, circle, fill=red] {};
            \node at (1.5,1) [draw, circle, fill=red] {};
                % Red node
            \node at (2.4,.25) [draw, circle, fill=red] {};
            %\node (arrival) at (.5,0) [draw, circle, fill=blue] {} node [below=.3cm, right] {\tiny arrival};
        % Second generation
        \draw (5,-.5) rectangle (8,1.5);
            % Line
            \draw (5.25,0) -- (7.75,0);
            % End ticks
            \draw [very thick] (5.25,-.25) -- (5.25,.25);
            \draw [very thick] (7.75,-.25) -- (7.75,.25);
            % Mid ticks
            \draw [thick] (6,-.125) -- (6,.125);
            \draw [thick] (7,-.125) -- (7,.125);
            % Dots
                % First node
            \node at (5.625,.25) [draw, circle, fill=blue] {};
            \node at (5.625,.5) [draw, circle, fill=blue] {};
            \node at (5.625,.75) [draw, circle, fill=red] {};
                % Second node
            \node at (6.5,.25) [draw, circle, fill=red] {};
                % Red node
            \node at (7.4,.25) [draw, circle, fill=red] {};
            \node at (7.4,.5) [draw, circle, fill=red] {};
            \node at (7.4,.75) [draw, circle, fill=red] {};
            \node at (7.4,1) [draw, circle, fill=blue] {};
        % Third generation
        \draw (10,-.5) rectangle (13,1.5);
            % Line
            \draw (10.25,0) -- (12.75,0);
            % End ticks
            \draw [very thick] (10.25,-.25) -- (10.25,.25);
            \draw [very thick] (12.75,-.25) -- (12.75,.25);
            % Mid ticks
            \draw [thick] (11,-.125) -- (11,.125);
            \draw [thick] (12,-.125) -- (12,.125);
            % Dots
                % First node
            \node at (10.625,.25) [draw, circle, fill=blue] {};
            \node at (10.625,.5) [draw, circle, fill=blue] {};
            \node at (10.625,.75) [draw, circle, fill=blue] {};
                % Second node
            \node at (11.5,.25) [draw, circle, fill=red] {};
            \node at (11.5,.5) [draw, circle, fill=red] {};
                % Red node
            \node at (12.4,.25) [draw, circle, fill=red] {};
            \node at (12.4,.5) [draw, circle, fill=red] {};
            \node at (12.4,.75) [draw, circle, fill=red] {};
        % Arrows
            \draw [->] (3.5,.5) -- (4.5,.5);
            \draw [->] (8.5,.5) -- (9.5,.5);
    \end{tikzpicture}
...
\frame{
        Generate population for all sources\;\\
        \While{$\text{clock} < \text{simulation time}$}{
            {Randomly group agents from each source\;}\\
            {Run a simulation of process\;}\\
            {Identify worst performing agents\;}\\
            {Reproduce agents with mutation\;}
            }
}

\frame{
	\centering
		\includegraphics[width=.8\textwidth]{./Images/damaging_behaviour_lambda.pdf}
}
