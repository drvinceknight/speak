...
\frame{
    \begin{center}
        \Huge\href{https://cloud.sagemath.com/projects/3b2249d5-a951-4f25-979a-571f71ccdde6/files/}{Sagemath}
    \end{center}
}
...
\frame{
    \begin{center}
        \includegraphics[width=.8\textwidth]{Images/old_talk-2-q-model.png}
    \end{center}
}
...
\[
    \epsilon(n) = \begin{cases}
        1/\mu,& n < c\\
        \frac{1 + n}{c\mu},& \text{otherwise}
    \end{cases}
\]

Cost of a state when using \(T\).

\[
    C_t(i,j) = \begin{cases}
        \epsilon_1(i) + \only<1>{\epsilon_2(j)}\only<2>{?}, t=0\\
        \beta_1+\epsilon_2(j), t=1\\
        \beta_2, t=2
    \end{cases}
\]
}
...
\frame{
A Nash policy \(\tilde T\) is a solution to:

\[
\min_{t\in\{0,1,2\}}C_t(i,j)=C_{T_{ij}}(i,j)\text{ for all }i,j
\]
}

\frame{
For example \(\mu=(3,1)\), \(c=(4,2)\), \(\beta=(.55,4)\) and \(\Lambda=2\):
...
\frame{
    \begin{center}
        \includegraphics[width=.8\textwidth]{Images/old_talk-2-q-poa.png}
    \end{center}
}
